<div class="mt-6 container mx-auto px-2 md:px-4">
    ...
    @if($post->image() !== null)
        <x-image alt="{{ $post->title }}" src="{{ $post->image() }}" class="my-10 w-full aspect-video shadow-md rounded-[2rem] rounded-bl-none z-0 object-cover"/>
    @endif
    ...
    <div class="bg-white dark:bg-gray-800 rounded-[2rem] rounded-tl-none shadow-md px-10 pb-6 ">
        ...
        <div class="flex flex-col items-start justify-start gap-4">
            ...
            <a href="#" class="flex items-center gap-2">
                <x-image src="{{ \Filament\Facades\Filament::getUserAvatarUrl($post->author) }}" alt="avatar" class="object-cover w-10 h-10 rounded-full sm:block"/>
